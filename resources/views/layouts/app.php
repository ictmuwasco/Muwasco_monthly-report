...
$navItems = [
    ...
    ['Months',      'months.php',   'M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5', false],
...
<body class="h-full bg-gray-100 font-sans antialiased text-gray-800">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-64 -translate-x-full flex-col bg-primary-dark text-white transition-transform duration-200 lg:static lg:translate-x-0 print:hidden">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4" aria-label="Main navigation">
            <ul class="space-y-1">
                <?php foreach ($navItems as [$label, $href, $iconPath, $adminOnly]): ?>
                    ...
                    <li>
                        <a href="<?= $href ?>"
                           class="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors
                                  <?= $currentPage === $href ? 'bg-white/15 text-white shadow-sm'
                                                             : 'text-cyan-100/75 hover:bg-white/10 hover:text-white' ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.7"
                                 stroke="currentColor" class="h-5 w-5 shrink-0 opacity-90 group-hover:opacity-100">
    ...
    <div class="flex min-w-0 flex-1 flex-col">
        ...
        <header class="sticky top-0 z-20 flex h-14 items-center gap-4 border-b border-gray-200 bg-white/90 px-4 backdrop-blur md:px-6 print:hidden">
            ...
            <nav class="hidden items-center gap-1.5 text-sm text-gray-400 sm:flex" aria-label="Breadcrumb">
                <span>MUWASCO</span><span>/</span>
                <span class="font-medium text-gray-700"><?= htmlspecialchars(str_replace('.php', '', $currentPage) ?: 'Dashboard') ?></span>
            </nav>
